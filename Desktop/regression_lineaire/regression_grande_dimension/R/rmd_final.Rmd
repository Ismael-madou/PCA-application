...
## Analyse sur les 2 et 3 eme axe
...
### Interprétation du Troisieme Axe avec dim 1-3 (Dim 3 :15.65 %)

  Sur cette troisieme dimension, la variable "super_plast" contribue le plus avec une valeur de *3.5%* et un niveau de representativité de *0.05*. De plus, elle evolue dans le meme sens que la variable cible "y_concrete_compressive qui n'est rien d'autres que la resistance à la compression du beton.
  Puis vient la variable "age" avec une contribution de *2.9%* et une qualité de representaion à la hauteur de *0.04*.Par contre, l'age évolue dans le sens négatif de celui de la variable cible. 
  Ensuite, nous passons à la variable "water" avec un niveau de contribution à la construction de cet axe à *1.4* et une qualité de representation d'une valeur qui tourne autour de *0.02*,comme dans le cas des deux premieres dimensions c'est-à-dire que son orientation négative indique qu’une teneur élevée en eau tend à être associée à une baisse de la résistance à la compression. 
  La variable "blast_furnace" est quatrième en terme de contribution avec une valeur de *0.48* et une qualité de representation de *0.6* qui est superieure à celles de toutes les variables déjà citées, ce qui signifie qu'une augmentation du laitier de haut fourneau diminue la resistance à la compression du beton. 
  De plus, nous avons la variable " coarse" qui est egalement du coté opposé de la variable cible "y_concrete_compressive" avec un niveau de contribution de *0.3* et une qualité de representation d'environ *0.42*. Cela sous entend que la hausse de cette variable a un effet negatif sur la resistance du beton. 
  En fin, la fleche representant la variable "cement" suit egalement la meme direction à celle de la variable "coarse", ce qui temoigne de leur forte correlation et par consequent à la conclusion selon laquelle l'augmentation du ciment a un impact negatif sur la resistance (logique en depassant le seuil necessaire). 
  Pour cette partie, nous nous sommes attardés à l'interpretation des variables qui contribuent en grande partie à la construction de cet axe. Par ailleurs, nous avons d'autres variables qui contribuent à la construction de cet axe de manière négligeable. c'est la raison pour laquelle, nous nous sommes pas attardés à leurs explications. En somme, cette dimension oppose d'un coté les variables "water", "blast_furnace", "age", "coarse" contre les variables "super_plast" et la variable cible "y_concrete_compressive".
...
```
## Interprétation du Troisième Axe avec dim 2-3

  Ces deux axes forment environ 37% de la variance expliquée. L'analyse de ce cercle de correlation indique que la troisième dimension oppose d'une part les variables "coarse" et "water" contre les variables "blast_furnace", "super_plast", "cement", "age" et "y_concrete_compressive" (variable cible). De plus, nous remarquons que la fleche de la variable "age" est confondue à celle de la variable "cement", cela signifie que les deux variables sont succeptibles d'influer de la meme façon sur la variable "cible". 
  
  
    
## Implications

Dans les deux cas, "super_plast" est opposée aux variables "water" et "coarse" c'est-à-dire qu'il s'avère important de regarder la relation qui existe entre le produit chimique et l'eau mais aussi les granulats grossiers.Cette opposition confirme les resultats des analyses portées sur le cercle de corélation de dimension 1-2. Contrairement au resultat de l'analyse de la dimension 1-2, A ce niveau l'analyse nous renseigne sur une eventuelle correlation entre "coarse" et "cement", ce qui veut dire qu'à certain niveau  l'effet combiné des ces deux variables aura un effet positif cependant si on s'ecarte de cet equilibre, l'effet combiné pourrait etre negatif sur la variable cible.Par ailleurs, les variables comme "blast_furnace", "age" et "cement" qui évoluaient dans le sens contraire de la direction de la variable reponse dans le cas de la dim 1-3, evoluent dans le meme sens que la variable cible dans le cas de la dimension 2-3. Cela peut etre du au fait que ces variables aient evidemment un effet positif sur la variable reponse en dessous d'un certain et que l"effet devient negatif au délà de ce seuil. Le constat de la relation entre l'eau et la resistance à la compression est assez particulière, toutes nos analyses basées sur les trois axes montrent que l'eau à un effet negatif sur la resistance du beton.
